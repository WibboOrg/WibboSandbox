<template>
    <transition enter-active-class="duration-300" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="duration-300" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div class="absolute top-0 left-0 right-0 z-50 p-2 text-center rounded" :class="isError ? 'bg-red-800' : 'bg-green-800'" v-if="messages.length">
            <div v-for="(message, index) in messages" :key="'message' + index">{{ message }}</div>
